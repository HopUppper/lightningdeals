    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="pt-36 text-center">
          ...
          <Link to="/categories" className="text-accent hover:text-accent/80 mt-4 inline-block text-sm font-body">Browse all products</Link>
        </div>
        <Footer />
      </div>
    );
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-28 section-padding">
        <div className="container-tight">
          <Link to="/categories" className="text-sm text-muted-foreground hover:text-foreground transition-colors font-body inline-flex items-center gap-1.5">
          ...
          <div className="grid lg:grid-cols-2 gap-16 mt-10">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div className="glass-card p-16 flex items-center justify-center relative overflow-hidden aspect-square">
                {savings > 0 && (
                  <span className="absolute top-4 right-4 text-xs font-body font-medium text-accent bg-accent/10 px-3 py-1 rounded-full">
                    {savings}% off
                  </span>
                )}
              ...
              <div className="grid grid-cols-3 gap-3 mt-4">
                {[
                  ...
                  <div key={t.label} className="glass-card p-4 text-center">
                    ...
                    <span className="text-[11px] text-muted-foreground font-body">{t.label}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              {product.categories?.name && (
                <Link
                  ...
                  className="text-xs text-muted-foreground font-body uppercase tracking-wider hover:text-foreground transition-colors"
                >
                  {product.categories.name}
                </Link>
              )}
              ...
              {avgRating && (
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className="text-sm text-muted-foreground font-body">{avgRating} ({reviews.length} review{reviews.length !== 1 ? "s" : ""})</span>
                </div>
              )}

              <p className="text-muted-foreground mt-5 leading-relaxed font-body">{product.long_description || product.description}</p>
              ...
              <div className="flex items-baseline gap-4 mt-8">
                ...
                {product.price_original > product.price_discounted && (
                  <span className="text-lg text-muted-foreground line-through font-body">₹{product.price_original}</span>
                )}
              </div>

              {product.offer_label && (
                <p className="mt-3 text-sm font-medium text-accent font-body">{product.offer_label}</p>
              )}
              ...
              <div className="flex gap-6 mt-6 text-sm text-muted-foreground font-body">
              ...
              {features.length > 0 && (
                <div className="mt-12">
                  <h3 className="font-body font-semibold text-foreground mb-5">Features Included</h3>
          ...
          <div className="mt-24 border-t border-border pt-16">
            <div className="flex items-end justify-between mb-10">
              ...
              {avgRating && (
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground font-body">{reviews.length} review{reviews.length !== 1 ? "s" : ""}</p>
                </div>
              )}
            </div>
            ...
            {user && (
              <div className="glass-card p-6 mb-8">
                <h3 className="font-body font-semibold text-foreground mb-4 text-sm">Write a Review</h3>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-border bg-secondary px-4 py-3 text-foreground text-sm font-body placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent resize-none h-24"
                />
                <button
                  onClick={submitReview}
                  disabled={submittingReview}
                  className="btn-primary !py-2.5 !px-6 !text-sm mt-3"
                >
            ...
            {reviews.length > 0 ? (
              <div className="grid sm:grid-cols-2 gap-4">
                {reviews.map((r) => (
                  <div key={r.id} className="glass-card p-6">
                    ...
                    {r.comment && (
                      <p className="text-sm text-muted-foreground leading-relaxed mb-4 font-body">"{r.comment}"</p>
                    )}
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-foreground font-body">{r.profiles?.name || "User"}</span>
              ...
              <div className="glass-card p-12 text-center">
                ...
                <p className="text-muted-foreground font-body">No reviews yet. {user ? "Be the first to review!" : "Login to leave a review."}</p>
              </div>
            )}
          </div>
        </div>
