...
const PWAInstallPrompt = () => {
  ...
  useEffect(() => {
    const dismissed = localStorage.getItem("ld-pwa-dismissed");
    if (dismissed) return;
  ...
  const handleDismiss = () => {
    setShowBanner(false);
    localStorage.setItem("ld-pwa-dismissed", "true");
  };
    ...
    <div className="fixed bottom-4 left-4 right-4 z-50 mx-auto max-w-md animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-card border border-border rounded-2xl p-4 shadow-2xl flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-foreground">Install Lightning Deals</p>
